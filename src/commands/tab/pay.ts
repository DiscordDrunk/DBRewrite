import { Command } from "../../structures/Command";
import { getOrCreateTab, clearTab, addToTab, reevaluateTabBlock } from "../../database/tab";
import { getUserBalance, updateBalance } from "../../database/userInfo";
import type { ChatInputCommandInteraction} from "discord.js";
import { MessageFlags } from "discord.js";
import { config } from "../../providers/config";

export const command = new Command("paytab", "Pay off or reduce your tab.")
	.setCategory("🍻tabs")
	.setExecutor(async (int: ChatInputCommandInteraction) => {
		const userId = int.user.id;
		const guildId = int.guildId!;

		// -------------------------
		// Defer immediately to prevent timeout
		// -------------------------
		await int.deferReply({ flags: MessageFlags.Ephemeral });

		// -------------------------
		// Get and validate amount
		// -------------------------
		const amountToPay = int.options.getNumber("amount");
		if (!amountToPay || amountToPay <= 0) {
			await int.editReply({
				content: "⚠️ You must specify a valid amount to pay.",
			});
			return;
		}

		// -------------------------
		// Fetch tab and balance
		// -------------------------
		const tab = await getOrCreateTab(userId, guildId);
		const { balance } = await getUserBalance(userId);

		// -------------------------
		// Grace period check
		// -------------------------
		if (tab.isBlocked && tab.lastPaidAt) {
			const timeSinceLastPaid = Date.now() - new Date(tab.lastPaidAt).getTime();
			if (timeSinceLastPaid > config.tabConfig.paymentGracePeriod) {
				await int.editReply({
					content: `❌ Your tab has been blocked for exceeding the grace period of ${config.tabConfig.paymentGracePeriod / 1000 / 3600
					} hours. Please pay off your tab to proceed.`,
				});
				return;
			}
		}

		// -------------------------
		// Check if user has enough funds
		// -------------------------
		if (balance < amountToPay) {
			await int.editReply({
				content: `❌ You don't have enough funds. Your balance is **$${balance.toFixed(
					2
				)}**, but you tried to pay **$${amountToPay.toFixed(2)}**.`,
			});
			return;
		}

		// -------------------------
		// Partial vs full payment
		// -------------------------
		if (amountToPay >= tab.amount) {
			await updateBalance(userId, balance - tab.amount);
			await clearTab(userId, guildId);

			await int.editReply({
				content: `💰 You’ve successfully paid off your tab of **$${tab.amount.toFixed(2)}**. Thank you!`,
			});
		} else {
			const remaining = tab.amount - amountToPay;

			await updateBalance(userId, balance - amountToPay);
			await addToTab(userId, guildId, -amountToPay);

			await int.editReply({
				content:
					`⚠️ You’ve paid **$${amountToPay.toFixed(2)}** toward your tab.\n` +
					`You still owe **$${remaining.toFixed(2)}**.\n` +
					`Please pay the remaining balance within the next **${config.tabConfig.paymentGracePeriod / 1000 / 3600
					} hours** to avoid being blocked.`,
			});
		}

		// -------------------------
		// Reevaluate block status (auto-unblock if paid enough)
		// -------------------------
		await reevaluateTabBlock(userId, guildId);
	})
	.addNumberOption("amount", "Amount to pay toward your tab", true);
